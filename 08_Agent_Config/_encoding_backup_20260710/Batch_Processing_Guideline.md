...
Zotero (Layer 1: Source)
    â†?PDF Verification (Zotero storage exists, readable)
    â†?MinerU Output Verification (full.md exists, parseable)
    â†?Level 1 Screening (Literature Card created)
    â†?Decision Framework (Ignore / Keep Reference / Deep Read)
    â†?Literature Card (saved to 02_KnowledgeVault/01_Papers/)
    â†?(Optional) Paper Note (Level 2 â€?only if Deep Read)
    â†?Knowledge Node linking (Methods, Tasks, Datasets, Ideas)
    â†?(Optional) Paper Logic (Level 3 â€?only if Argument Mining triggers met)
```
...
2. **Zotero Item Key must be recorded** â€?this is the paper's immutable identity
3. **Better BibTeX citation key must be resolved** â€?used for manuscript citations
4. **Metadata verified** â€?title, authors, year, venue, DOI match expected values
...
### Level 3 â€?Argument Mining Paper Logic
...
|---|---|
| **Method** | Existing methods in `03_Methods/` â€?compare architecture, purpose, domain |
| **Task** | Existing tasks in `04_Tasks/` â€?compare input/output formulation |
| **Dataset** | Existing datasets in `05_Datasets/` â€?compare source, format, scale |
| **Idea** | Existing ideas in `07_Ideas/` â€?compare hypothesis, approach, feasibility |
...
## 5.6 Processing Gate ¡ª Existing KnowledgeVault Check
...
1. **MinerU_Zotero_Mapping.md** ¡ª Has this paper already been processed? Check Zotero Item Key and Paper ID columns.
2. **Paper_Index.md** ¡ª Is this paper listed in the vault index? Check filename patterns.
3. **01_Papers/ directory** ¡ª Does a file with the same Paper ID pattern exist? Check {author}{year}_{keyword}_{type}.md naming convention.
...
- Do NOT create a duplicate note ¡ª update the existing file instead.
