...
Literature Acquisition
  鈫?Paper Understanding
  鈫?Knowledge Organization
  鈫?Research Gap Discovery
  鈫?Model Development
  鈫?Experiment Management
  鈫?Result Analysis
  鈫?Scientific Writing
```
...
| Stage | Status |
|---|---|
| Stage 0 鈥?Workspace Initialization | 鉁?Completed |
| Stage 1.1 鈥?Obsidian KnowledgeVault Init | 鉁?Completed |
| Stage 1.2 鈥?Obsidian Note Templates | 鉁?Completed |
| Stage 1.3 鈥?Navigation & Knowledge Graph | 鉁?Completed |
| Stage 1.4A 鈥?First Paper Validation | 鉁?Completed |
| Stage 1.4A.1 鈥?Agent Bootstrap | 鉁?Completed |
| Stage 1.4B-0 鈥?Literature Intake System | 鉁?Completed |
| Stage 1.4B-1 鈥?Processed Markdown Pipeline | 鉁?Completed |
| Stage 1.4C-0 鈥?Zotero Integration Design | 鉁?Completed |
| Stage 1.4C-1 鈥?Zotero Deployment Prep | 鉁?Completed |
| Stage 1.4C-1.1 鈥?Design Principles & ID Fix | 鉁?Completed |
| Stage 1.4C-2 鈥?Zotero Test Plan | 鉁?Completed |
| Stage 1.4C-3 鈥?Zotero Readiness | 鉁?Completed |
| Stage 1.4C-3.1 鈥?Data Path Sync | 鉁?Completed |
| Stage 1.4C-3.2 鈥?Zotero Status Sync | 鉁?Completed |
| Stage 1.4C-3.3 鈥?Storage Confirmation | 鉁?Completed |
| **Stage 1.4C-3.4 鈥?PDF Architecture Redesign** | 馃攧 Current |
| Stage 1.4B 鈥?Survey Template & Data Setup | 鈴?Next |
...
| Directory | Purpose |
|---|---|
| `00_Inbox/` | Temporary input area 鈥?new papers, notes, unprocessed files |
| `01_Literature/` | Literature management 鈥?MinerU output, processed markdown, index |
| `02_KnowledgeVault/` | Obsidian knowledge base 鈥?the intellectual memory of the project |
| `03_Projects/` | Research implementation 鈥?DL code, training scripts, configs |
| `04_Tools/` | Reusable scripts 鈥?preprocessing, conversion, visualization |
| `05_Experiments/` | Experiment tracking 鈥?configs, results, figures |
| `06_Writing/` | Scientific writing 鈥?drafts, manuscripts, proposals |
| `07_Research_Ideas/` | Idea management 鈥?gaps, hypotheses, future directions |
| `08_Agent_Config/` | AI agent config 鈥?instructions, skills, templates, workflows |
...
1. **Read `AGENT_BOOTSTRAP.md`** 鈥?understand project identity, constraints, and status.
2. **Read `PROJECT_STATUS.md`** 鈥?understand completed stages and next planned work.
3. **Read `README.md`** 鈥?understand overall project design and principles.
4. **Read the current task document** 鈥?understand what you are being asked to do.
...
- **Confirmed Available** — verified the repository exists and is reachable
...
1. Read PROJECT_STATUS.md — understand completed stages and current position.
2. Read Current_State_Check.md — understand known issues and next actions.
3. Read ResearchAI_Design_Principles.md — remember permanent architectural decisions.
4. Read relevant ADR documents (e.g., ADR_Zotero_PDF_Centered_Architecture.md) — remember why specific architecture choices were made.
...
- **Mentioned in paper** 鈥?a dataset the paper references but did not personally use
- **Personally used** 鈥?a dataset the paper actually ran experiments on
- **Benchmark target** 鈥?a dataset used as a standard evaluation target
...
- **Zotero data directory**: `D:\ResearchAI_Data\`
  - `storage/` 鈥?PDF files (MinerU reads from here)
  - `zotero.sqlite` 鈥?bibliographic metadata
